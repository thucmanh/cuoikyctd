...
Type *compileExpression3(void)
{
  ...
  case SB_PLUS:
    ...
    checkBasicType(type1);
    // if (type1->typeClass == TP_FLOAT || type1->typeClass == TP_INT)
    //   checkNumberType(type1);
    // else if (type1->typeClass == TP_STRING)
    //   checkStringType(type1);
    ...
    if (type2 != NULL)
    {
      ...
      checkTypeEquality(type1, type2);
      // if (type2->typeClass == TP_FLOAT || type2->typeClass == TP_INT)
      //   checkNumberType(type2);
      // else if (type2->typeClass == TP_STRING)
      //   checkStringType(type2);
    }
    return type1;
    break;
  case SB_MINUS:
